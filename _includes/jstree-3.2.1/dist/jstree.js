 ...
 */
	...
	$.jstree.core.prototype = {
		...
		redraw_node : function (node, deep, is_callback, force_render) {
			...
			//my!
			node.setAttribute('flag', obj.flag);
			//!
		...
		items : function (o, cb) { // Could be an object directly
			return {
				...
				"rename" : {
					...
					"shortcut"			: 113,
					"shortcut_label"	: 'F2',
					"icon"				: "glyphicon glyphicon-erasearia-labelledby",
 ...
 */
	...
	$.jstree.defaults.unique = {
		...
		case_sensitive : false,
		/**
		 * A callback executed in the instance's scope when a new node is created and the name is already taken, the two arguments are the conflicting name and the counter. The default will produce results like `New_node (2)`.
	...
	$.jstree.plugins.unique = function (options, parent) {
		...
		this.create_node = function (par, node, pos, callback, is_loaded) {
			if(!node || node.text === undefined) {
				...
				n = tmp = this.get_string('New_node');
				dpc = [];
				for(i = 0, j = par.children.length; i < j; i++) {
